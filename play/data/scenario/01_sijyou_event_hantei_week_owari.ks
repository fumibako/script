...
;ennding パラメータは適当です
;================================================
;◆四条normalED判定 四条ルートかつ3月4週が終わる時点で淑女度一定値未満又は好感度一定値未満で1度だけ発生
[if exp="(f.okeiko_month==3 && f.okeiko_week==4) && f.event_sijyou[16]==0 && f.sijyou_au==1 && (f.para_sijyou_koukando <= 80 || f.para_shujinkou_shukujodo <= 80)"]
	[eval exp="f.event_storage='sijyou_3_4_goodED.ks'"]
...
[if exp="(f.okeiko_month==3 && f.okeiko_week==4) && f.event_sijyou[17]==0 && f.sijyou_au==1 && (f.para_sijyou_koukando > 80 && f.para_shujinkou_shukujodo > 80)"]
	[eval exp="f.event_storage='sijyou_3_4_epilogue_goodED.ks'"]
