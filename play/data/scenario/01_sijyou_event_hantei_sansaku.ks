...
;散策イベントを見たフラグをイベントファイル末に移動しました(イベント発生処理後に読込不良がある場合に、本当は見ていないのに「見た」フラグだけ立って散策イベントを見られない場合があったため)
...
[wait time=50]
;↓f.event_sijyou[5] == 1を条件に入れるとなぜか判定スルーされてしまう場合が多いため、かつf.event_sijyou[5]は仕様上、散策3の前提とは思えないため条件からf.event_sijyou[5]を抜きます◆jsYiJcqRkk
[if exp="f.sijyou_au == 1 && ((f.okeiko_month == 9 && f.okeiko_week == 4)||(f.okeiko_month == 10 && f.okeiko_week == 1)) && f.event_machi_sijyou[3] == 0"]
[wait time=50]
...
;sijyou_sansaku13.ks
;↓他のイベント判定処理リストに戻ります
@jump storage="sansaku_hantei_kobetsu.ks" target=*sijyou_event_hantei_owari
