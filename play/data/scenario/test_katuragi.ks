...
;[if exp="sf.replay_katuraginomiya == 1 && sf.event_katuragi_event_1 == undefined"]
;↑リプレイやおまけモードで名前表示に使われているsf変数があるのでこれだけ[eval exp="sf.event_katuragi_event_1 = 1"]と設定できます。
;しかし、ここで行いたいのは新規プレイヤーさんの為のデバックなので使用しておりません。グッドエンドやノーマルエンド等で1にする場合は条件等を変更してください。
...
[ptext name="num_time1" text=&tf.event_Number_of_times layer=26 size=17 x=300 y=85 color=darkslateblue overwrite=true]
[ptext text="/17" layer=26 size=17 x=350 y=85 color=darkslateblue]
