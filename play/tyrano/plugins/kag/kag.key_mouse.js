tyrano.plugin.kag.key_mouse = {
  ...
  init: function () {
    ...
    $(document).keydown(function (e) {
      if (that.is_keydown == true) return false;
      that.is_keydown = true;
      var keycode = e.keyCode;
      if (that.map_key[keycode])
        if (typeof that.map_key[keycode] == "function") that.map_key[keycode]();
        else if (that[that.map_key[keycode]]) that[that.map_key[keycode]]()
    });
    $(document).keyup(function (e) {
      that.is_keydown = false;
      var keycode = e.keyCode;
      if (keycode == 91 || keycode == 17) that.kag.stat.is_skip = false
    });
    $(document).on("mousedown", function (e) {
      ...
      var target = null;
      if (e.which == 2) target = that.map_mouse["center"];
      else if (e.which == 3) target = that.map_mouse["right"];
      if (typeof target == "function") target();
      else if (that[target]) that[target]()
    });
    var mousewheelevent = "onwheel" in document ? "wheel" : "onmousewheel" in document ? "mousewheel" : "DOMMouseScroll";
    $(document).on(mousewheelevent, function (e) {
      if (!that.canShowMenu()) return;
      if ($(".menu_close").size() > 0 && $(".layer_menu").css("display") != "none") return;
      var delta = e.originalEvent.deltaY ? -e.originalEvent.deltaY : e.originalEvent.wheelDelta ? e.originalEvent.wheelDelta : -e.originalEvent.detail;
      var target = null;
      if (delta < 0) target = that.map_mouse["wheel_down"];
      else target = that.map_mouse["wheel_up"];
      if (typeof target == "function") target();
      else if (that[target]) that[target]()
    });
      ...
      layer_obj_click.swipe({
        swipe: function (event, direction, distance, duration, fingerCount, fingerData) {
          ...
          if (that.map_ges[swipe_str])
            if (that[that.map_ges[swipe_str]["action"]]) that[that.map_ges[swipe_str]["action"]]();
    ...
    layer_obj_click.click(function (e) {
      ...
      if (that.kag.stat.is_click_text == true) return false;
  ...
  showmenu: function () {
    if (this.canShowMenu())
      ...
      else $(".button_menu").click()
  },
  hidemessage: function () {
    if (this.canShowMenu())
      if ($(".menu_close").size() > 0 && $(".layer_menu").css("display") != "none") $(".menu_close").click();
      else if (!this.kag.stat.is_strong_stop)
      if (this.kag.stat.is_hide_message) this.kag.layer.showMessageLayers();
      else this.kag.ftag.startTag("hidemessage")
  },
  save: function () {
    this._role("save")
  },
  ...
  skip: function () {
    if (this.canClick()) this._role("skip")
  },
  backlog: function () {
    this._role("backlog")
  },
  ...
  _role: function (role) {
    ...
    if (that.kag.stat.enable_keyconfig == false) return false;
    that.kag.stat.is_skip = false;
    if (role != "auto") that.kag.ftag.startTag("autostop", {});
    ...
    switch (role) {
    case "save":
      if ($(".layer_menu").css("display") == "none") that.kag.menu.displaySave();
      break;
    case "load":
      if ($(".layer_menu").css("display") == "none") that.kag.menu.displayLoad();
      break;
    case "window":
  ...
  canClick: function () {
    if ($(".layer_event_click").css("display") != "none" && $(".layer_menu").css("display") == "none") return true;
    return false
  },
