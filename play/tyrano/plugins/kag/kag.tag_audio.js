tyrano.plugin.kag.tag.playbgm = {
  ...
  play: function (pm) {
    ...
    if (pm.target == "se") {
      ...
      if (this.kag.stat.map_vo["vobuf"][pm.buf]) this.kag.tmp.is_vo_play = true
    } else {
      this.kag.tmp.is_audio_stopping = false;
      this.kag.tmp.is_bgm_play = true
    }
    var volume = 1;
    if (pm.volume !== "") volume = parseFloat(parseInt(pm.volume) / 100);
    else if (target === "bgm") {
      if (typeof this.kag.config.defaultBgmVolume == "undefined") volume = 1;
      else volume = parseFloat(parseInt(this.kag.config.defaultBgmVolume) / 100);
      if (typeof this.kag.stat.map_bgm_volume[pm.buf] != "undefined") volume = parseFloat(parseInt(this.kag.stat.map_bgm_volume[pm.buf]) / 100)
    } else {
      if (typeof this.kag.config.defaultSeVolume == "undefined") volume = 1;
      else volume = parseFloat(parseInt(this.kag.config.defaultSeVolume) / 100);
      if (typeof this.kag.stat.map_se_volume[pm.buf] != "undefined") volume = parseFloat(parseInt(this.kag.stat.map_se_volume[pm.buf]) / 100)
    }
    var storage_url = "";
    ...
    else if (storage != "") storage_url = "./data/" + target + "/" + storage;
    else storage_url = "";
    var audio_obj = null;
    ...
    else if (this.kag.tmp.map_se[pm.buf] != null) {
  ...
  playAudio: function (audio_obj, pm, target) {
    ...
    if (pm.volume != "") volume = parseFloat(parseInt(pm.volume) / 100);
      ...
      if (!this.kag.config.defaultBgmVolume) volume = 1;
      else volume = parseFloat(parseInt(this.kag.config.defaultBgmVolume) / 100);
    else if (!this.kag.config.defaultSeVolume) volume = 1;
    else volume = parseFloat(parseInt(this.kag.config.defaultSeVolume) / 100);
...
tyrano.plugin.kag.tag.fadeinbgm = {
  ...
  start: function (pm) {
    if (parseInt(pm.time) <= 100) pm.time = 100;
...
tyrano.plugin.kag.tag.bgmopt = {
  ...
  start: function (pm) {
    ...
    if (pm.effect == "true" && this.kag.define.FLAG_APRI == false) {
      ...
      if (pm.buf == "")
        for (key in map_bgm) {
          if (map_bgm[key]) map_bgm[key].volume = new_volume
        } else if (map_bgm[pm.buf]) map_bgm[pm.buf].volume = new_volume
    }
...
tyrano.plugin.kag.tag.seopt = {
  ...
  start: function (pm) {
    ...
    if (pm.effect == "true" && this.kag.define.FLAG_APRI == false) {
      ...
      if (pm.buf == "")
        for (key in map_se) {
          if (map_se[key]) map_se[key].volume = new_volume
        } else if (map_se[pm.buf]) map_se[pm.buf].volume = new_volume
    }
    this.kag.ftag.startTag("eval", {
...
tyrano.plugin.kag.tag.wbgm = {
  pm: {},
  start: function () {
    if (this.kag.tmp.is_bgm_play == true) this.kag.tmp.is_bgm_play_wait = true;
...
tyrano.plugin.kag.tag.wse = {
  pm: {},
  start: function () {
    if (this.kag.tmp.is_se_play == true) this.kag.tmp.is_se_play_wait = true;
    else this.kag.ftag.nextOrder()
  }
};
tyrano.plugin.kag.tag.voconfig = {
  ...
  start: function (pm) {
    ...
    if (pm.name != "") {
      var vochara = {};
      if (this.kag.stat.map_vo["vochara"][pm.name]) vochara = this.kag.stat.map_vo["vochara"][pm.name];
      else vochara = {
        "vostorage": "",
        "buf": pm.sebuf,
        "number": 0
      ...
      if (pm.number != "") vochara["number"] = pm.number;
