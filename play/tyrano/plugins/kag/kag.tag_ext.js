...
tyrano.plugin.kag.tag.movie = {
    ...
    start: function(pm) {
        ...
        if ($.userenv() != "pc") {
            ...
            if ($.getBrowser() == "firefox" || $.getBrowser() == "opera") pm.storage = $.replaceAll(pm.storage, ".mp4", ".webm");
            that.playVideo(pm)
        }
    },
    playVideo: function(pm) {
        ...
        else if (typeof this.kag.config.defaultMovieVolume != "undefined") video.volume = parseFloat(parseInt(this.kag.config.defaultMovieVolume) / 100);
        ...
        if (pm.skip == "true") video.addEventListener("click", function(e) {
            $(".tyrano_base").find("video").remove();
            that.kag.ftag.nextOrder()
        });
...
tyrano.plugin.kag.tag.autostart = {
    pm: {},
    start: function(pm) {
        if (this.kag.stat.is_auto == true) return false;
...
tyrano.plugin.kag.tag.chara_hide = {
    ...
    start: function(pm) {
            ...
            complete: function() {
                ...
                if (that.kag.stat.chara_pos_mode == "true") {
                    ...
                    $(array_tyrano_chara).each(function() {
                        ...
                        var left = tmp_base - center;
                        if (that.kag.stat.chara_anim ==
                            "false") j_chara.fadeTo(parseInt(pm.time), 0, function() {
                            j_chara.css("left", left);
                            j_chara.fadeTo(parseInt(pm.time), 1, function() {
                                chara_num--;
                                if (chara_num == 0) {
        ...
        if (this.kag.stat.chara_memory ==
            "false") this.kag.stat.charas[pm.name].storage = this.kag.stat.charas[pm.name]["map_face"]["default"];
