tyrano.plugin.kag.ftag = {
    ...
    checkCw: function(tag) {
        ...
        if (master_tag.cw)
            if (this.kag.stat.is_script != true && this.kag.stat.is_html != true && this.kag.stat.checking_macro != true) return true;
            else return false;
        else return false
    },
    ...
    checkVital: function(tag) {
        ...
        for (var i = 0; i < array_vital.length; i++)
            if (tag.pm[array_vital[i]]) {
                if (tag.pm[array_vital[i]] == "") err_str += "\u30bf\u30b0\u300c" + tag.name + "\u300d\u306b\u30d1\u30e9\u30e1\u30fc\u30bf\u30fc\u300c" +
                    array_vital[i] + "\u300d\u306f\u5fc5\u9808\u3067\u3059\u3000\n"
    ...
    nextOrderWithLabel: function(label_name,
        scenario_file) {
    ...
    nextOrderWithIndex: function(index, scenario_file, flag, insert, auto_next) {
        ...
        if (scenario_file != this.kag.stat.current_scenario || flag == true) {
            this.kag.layer.hideEventLayer();
            this.kag.loadScenario(scenario_file,
                function(array_tag) {
...
tyrano.plugin.kag.tag.text = {
    ...
    showMessage: function(message_str) {
        ...
        (function(jtext) {
            ...
            var pchar = function(pchar) {
                var c = _message_str.substring(index,
                    ++index);
    ...
    showMessageVertical: function(message_str) {
        ...
        (function(jtext) {
            ...
            var j_span = that.kag.getMessageCurrentSpan();
            j_span.css("color",
                that.kag.stat.font.color).css("font-weight", that.kag.stat.font.bold).css("font-size", that.kag.stat.font.size + "px").css("font-family", that.kag.stat.font.face);
            if (that.kag.config.autoRecordLabel == "true")
                if (that.kag.config.alreadyReadTextColor != "default")
                    if (that.kag.stat.already_read == true) j_span.css("color", $.convertColor(that.kag.config.alreadyReadTextColor));
            var ch_speed = 30;
            if (that.kag.stat.ch_speed != "") ch_speed = parseInt(that.kag.stat.ch_speed);
            else if (that.kag.config.chSpeed) ch_speed = parseInt(that.kag.config.chSpeed);
            var pchar = function(pchar) {
...
tyrano.plugin.kag.tag.position = {
    ...
    start: function(pm) {
        ...
        if (pm.marginr != "0") new_style_inner["width"] = parseInt(layer_inner.css("width")) - parseInt(pm.marginr) +
            "px";
...
tyrano.plugin.kag.tag.image = {
    ...
    start: function(pm) {
        ...
        if (pm.layer != "base") {
            ...
            if ($.isHTTP(pm.storage)) strage_url = pm.storage;
            else strage_url = "./data/" + folder + "/" + pm.storage;
            var img_obj = $("<img />");
        ...
        if (pm.layer != "base") {
            ...
            if (pm.time != "") {
                ...
                j_obj.animate({
                    ...
                    cnt++;
                    if (s_cnt == cnt) that.kag.ftag.nextOrder()
                })
            } else {
                that.kag.layer.getLayer(pm.layer, pm.page).empty();
                that.kag.ftag.nextOrder()
            ...
            this.kag.layer.getLayer(pm.layer,
                pm.page).css("background-image", "");
            this.kag.ftag.nextOrder()
        }
    }
};
...
tyrano.plugin.kag.tag.mtext = {
    ...
    start: function(pm) {
        ...
        var font_new_style = {
             "text-shadow": "-1px -1px 1px rgba(255,255,255,0.9), 1px -1px 1px rgba(255,255,255,0.9), 1px 1px 1px rgba(255,255,255,0.9), -1px 1px 1px rgba(255,255,255,0.9), -1px -1px 0px rgba(255,255,255,0.9), 1px -1px 0px rgba(255,255,255,0.9), 1px 1px 0px rgba(255,255,255,0.9), -1px 1px 0px rgba(255,255,255,0.9), 0px 0px 10px rgba(255,255,200,0.9), 0px 0px 5px rgba(255,255,255,1)",
           "color": pm.color,
            "font-weight": pm.bold,
            "font-style": pm.fontstyle,
            "font-size": pm.size + "px",
                    ...
                    switch (_pm.role) {
                        ...
                        case "auto":
                            if (that.kag.stat.is_auto ==
                                true) that.kag.ftag.startTag("autostop", {});
