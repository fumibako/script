...
tyrano.plugin.kag.tag.text = {
  ...
  showMessage: function (message_str, pm) {
    ...
    } else if (pm.backlog == "join") this.kag.pushBackLog(message_str, "join");
    else this.kag.pushBackLog(message_str, "add");
    ...
    (function (jtext) {
      ...
      var pchar = function (pchar) {
        ...
        current_str += c;
        if (that.kag.stat.is_skip != true && that.kag.stat.is_nowait != true && ch_speed > 3) that.kag.appendMessage(jtext, current_str);
  ...
  showMessageVertical: function (message_str, pm) {
    ...
    that.kag.ftag.hideNextImg();
    if (pm.backlog == "join") this.kag.pushBackLog(message_str, "join");
    else this.kag.pushBackLog(message_str, "add");
    (function (jtext) {
...
tyrano.plugin.kag.tag.image = {
  ...
  start: function (pm) {
    ...
    if (pm.layer != "base") {
      ...
      if (pm.zindex != "") img_obj.css("z-index", pm.zindex);
...
tyrano.plugin.kag.tag.layermode = {
  ...
  start: function (pm) {
    ...
    else blend_layer.fadeIn(parseInt(pm.time), function () {
      if (pm.wait == "true") that.kag.ftag.nextOrder()
    });
    if (pm.wait == "false") this.kag.ftag.nextOrder()
  }
